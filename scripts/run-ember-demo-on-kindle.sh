#!/usr/bin/env bash
# Copy the latest ember-demo binary to a jailbroken Kindle over SSH and
# launch it in the foreground app slot.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DEFAULT_BIN="$REPO_ROOT/apps/com.bd452.emberdemo/package/bin/kindlehf/ember-demo"
FALLBACK_BIN="$REPO_ROOT/target-kindle/armv7-unknown-linux-gnueabihf/release/ember-demo"

BIN="${BIN:-$DEFAULT_BIN}"
KINDLE="${KINDLE:-${KINDLE_HOST:-}}"
KINDLE_USER="${KINDLE_USER:-root}"
KINDLE_PASSWORD="${KINDLE_PASSWORD:-kindle}"
REMOTE_BIN="${REMOTE_BIN:-/mnt/us/ember-demo}"
...
Usage:
  ...
  BIN=target-kindle/armv7-unknown-linux-gnueabihf/release/ember-demo $0
...
"${ssh_cmd[@]}" "$TARGET" 'killall ember-demo 2>/dev/null || true; sleep 1'
...
cd /mnt/us
nohup '$REMOTE_BIN' >/mnt/us/ember-demo.out 2>/mnt/us/ember-demo.err &
sleep 2
ps | grep ember-demo | grep -v grep || true
if [ -s /mnt/us/ember-demo.err ]; then
    echo '--- /mnt/us/ember-demo.err ---'
