...
//!
//! This runs only when the `ember` crate is built with `--features fbink`
...
fn main() {
    ...
    let repo_root = manifest_dir
        .parent()
        .expect("fbink-sys must live at <repo>/fbink-sys")
        .to_path_buf();
    ...
    // The ember crate is also built as a cdylib (libember.so, the C
