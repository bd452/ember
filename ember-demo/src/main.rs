...
impl Component for DemoScreen {
    fn build(&mut self, ctx: &mut BuildCtx) -> Node {
        ...
        let counter = Label::new("").size(3);
        {
            ...
            ctx.bind(&self.count, move |v| {
                counter.set_text(format!("Count: {v}"))
            });
        }
        ...
        Node::Stack {
            ...
            bg: Some(Color::WHITE),
            children: vec![
                Label::new("Ember Demo").size(4).into_node(),
                counter.into_node(),
                buttons,
...
fn main() -> std::io::Result<()> {
    ...
    app.render_frame()?;
    println!("ember-demo host stub: rendered one frame (no FBInk).");
    Ok(())
}
