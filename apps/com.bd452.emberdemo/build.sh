#!/usr/bin/env bash
# Cross-compile the Ember demo binary for each Kindle platform and stage it
# into the package. Modeled on ../com.bd452.demo/build.sh. The binary links both
# ember and FBInk statically, so the package has no runtime dependencies.
set -euo pipefail
...
build_platform() {
    ...
    echo "==> Building ember-demo for $platform ($rust_target)"
    ...
    # Honor CARGO_TARGET_DIR (e.g. target-kindle from build-in-container.sh).
    local target_dir="${CARGO_TARGET_DIR:-$RUST_DIR/target}"
    install -m 755 "$target_dir/${rust_target}/release/ember-demo" \
        "$APP_ROOT/package/bin/${platform}/ember-demo"
}
