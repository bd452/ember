#!/usr/bin/env bash
# Cross-compile libember.so (with the C ABI + FBInk backend) for each Kindle
...
build_platform() {
    ...
    echo "==> Building libember for $platform ($rust_target)"
    ...
    install -m 644 "$target_dir/${rust_target}/release/libember.so" \
        "$APP_ROOT/package/lib/${platform}/libember.so"
}

build_platform kindlehf
build_platform kindlepw2

cp "$RUST_DIR/ember/include/ember.h" "$APP_ROOT/package/include/ember.h"
