...
cd rust
cargo test -p ember
cargo test -p ember --features capi
...
| `capi` | no | C ABI in `ffi.rs` / `libember.so` |
...
` .cargo/config.toml` names the linkers as bare tool names
...
platform, because Cargo does not always discover ` .cargo/config.toml`
...
| `CARGO_TARGET_DIR` | optional | e.g. `target-kindle` to keep host and cross artifacts separate |
...
- Use `make pic` (PIC static lib), not plain `static`, so `libember.so`
...
```sh
# Library: libember.so (capi+fbink) for both platforms + header
...
| Script | Artifacts |
|--------|-----------|
| ember `build.sh` | `apps/com.bd452.ember/package/lib/{kindlehf,kindlepw2}/libember.so`, `package/include/ember.h` |
| demo `build.sh` | `apps/com.bd452.emberdemo/package/bin/{kindlehf,kindlepw2}/ember-demo` |
| cargo (default) | `target/<triple>/release/…` |
...
# Optional: keep cross artifacts out of target
export CARGO_TARGET_DIR="$PWD/target-kindle"

cargo build --manifest-path Cargo.toml \
  -p ember-demo --release --features fbink --target "$rust_target"

# Binary:
#   $CARGO_TARGET_DIR/$rust_target/release/ember-demo
# or target/$rust_target/release/ember-demo
```
...
```sh
cargo build --manifest-path Cargo.toml \
  -p ember --release --features capi,fbink --target "$rust_target"
# → .../release/libember.so
```
...
`CARGO_TARGET_DIR=/repo/target-kindle`, and runs your command:
...
- Cargo output: `target-kindle/<triple>/release/` on the host (warm
...
BIN=target-kindle/armv7-unknown-linux-gnueabihf/release/ember-demo
# or: apps/com.bd452.emberdemo/package/bin/kindlehf/ember-demo
...
```sh
export EMBER_TOUCH_DEV=/dev/input/event2
export EMBER_TOUCH_SWAP=1
export EMBER_TOUCH_INVERT_X=1
export EMBER_TOUCH_INVERT_Y=1
export EMBER_TOUCH_RAW=WxH
```

## 7. Regenerating the C header

Committed header: `ember/include/ember.h`.

```sh
cd rust
cargo install cbindgen --locked   # once
cbindgen --crate ember --config ember/cbindgen.toml \
...
| `scp: dest open … Failure` | Binary still running on device | `killall ember-demo` then retry |
...
| Package `build.sh` can’t find binary | `CARGO_TARGET_DIR` vs `target` | Scripts honor `CARGO_TARGET_DIR`; rebuild helper/image docs |
...
|------|--------------------|
| Signals, layout, structural, app, input | `cargo test -p ember` |
| C ABI smoke | `cargo test -p ember --features capi` |
| Demo host stub | `cargo run -p ember-demo` |
