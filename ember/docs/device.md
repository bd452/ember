...
```toml
ember = { path = "../ember", features = ["fbink"] }
```

## Opening the app

```rust
use ember::render::fbink::FbinkRenderer;
use ember::{App, ExitHandle};
...
| Variable | Effect |
|----------|--------|
| `EMBER_TOUCH_SWAP=1` | Swap X/Y |
| `EMBER_TOUCH_INVERT_X=1` | Invert X after swap |
| `EMBER_TOUCH_INVERT_Y=1` | Invert Y after swap |
| `EMBER_TOUCH_RAW=WxH` | Force raw axis maxima |
...
on its own after a raw process exits. On exit, Ember (and the demo’s
`launch.sh`) call:
...
system manages foregrounding around sleep. Ember does not currently
...
fn main() -> std::io::Result<()> {
    use ember::geometry::Size;
    use ember::render::mock::MockRenderer;
    use ember::App;
...
This is what `ember-demo` does when built without `fbink`.
