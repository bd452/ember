...
Ember is a **build-once, mutate-in-place** UI framework. There is no virtual
DOM and no re-render of the component tree when state changes. That design comes
from the original Swift Ember (which sits on UIKit); the Kindle port keeps
...
single follow-up pass runs with the final value. That matches Swift Ember.
