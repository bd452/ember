...
impl BuildCtx<'_> {
    ...
    /// every change. Replaces Ember's keypath `bind` — `apply` is the setter
...
mod tests {
    ...
    impl Component for Counter {
        fn build(&mut self, ctx: &mut BuildCtx) -> Node {
            ...
            ctx.bind(&self.count, move |v| {
                bind_label.set_text(format!("Count: {v}"))
            });

            let inc = self.count.clone();
            let button = Button::new("+").on_tap(move || inc.update(|v| v + 1));

            vstack(8, Align::Fill, vec![label.into_node(), button.into_node()])
        }
    }
    ...
    fn binding_applies_current_value_at_mount() {
        ...
        let mounted = mount_component(Box::new(Counter {
            count: count.clone(),
        }));
