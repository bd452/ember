...
impl Calibration {
    ...
    /// `EMBER_TOUCH_SWAP=1`, `EMBER_TOUCH_INVERT_X=1`,
    /// `EMBER_TOUCH_INVERT_Y=1`, `EMBER_TOUCH_RAW=WxH`.
    ...
    fn apply_env_overrides(&mut self) {
        ...
        self.swap_xy = flag("EMBER_TOUCH_SWAP");
        self.invert_x = flag("EMBER_TOUCH_INVERT_X");
        self.invert_y = flag("EMBER_TOUCH_INVERT_Y");
        if let Ok(raw) = std::env::var("EMBER_TOUCH_RAW") {
...
///
/// Honors the `EMBER_TOUCH_DEV` override. Otherwise picks the first device
...
pub fn find_touch_device() -> Option<std::path::PathBuf> {
    ...
    if let Ok(dev) = std::env::var("EMBER_TOUCH_DEV") {
